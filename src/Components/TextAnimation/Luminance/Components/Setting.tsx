...
const LuminanceSetting = ({
  handleRenderVideo,
  contentHtml,
  imageBg,
}: LuminanceSettingProps) => {
  const {
    text,
    fontSize,
    fontFamily,
  ...
  const handleChangeKeyFrame = (fontColor: string) => {
    let keyframes = `
    @keyframes glow {
      40% {
        text-shadow: 0 0 8px ${fontColor};
      }
    }`
  ...
  useEffect(() => {
    handleChangeKeyFrame(fontColor)
  }, [fontColor])
...
export default LuminanceSetting
